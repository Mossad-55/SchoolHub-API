...
internal sealed class AssignmentService : IAssignmentService
{
    ...
    {
        _logger.LogDebug($"Fetching all assignments for batch {batchId}");
        ...
        _logger.LogInfo($"Fetched {assignmentDtos.Count()} assignments for batch {batchId}");
    ...
    {
        _logger.LogDebug($"Fetching assignment by id {id}");

        var assignmentEntity = await _repository.Assignment.GetByIdAsync(id, trackChanges);
        if (assignmentEntity is null)
        {
            _logger.LogWarn($"Assignment with id {id} not found");
            throw new AssignmentNotFoundException(id);
        }

        var dto = _mapper.Map<AssignmentDto>(assignmentEntity);
        _logger.LogInfo($"Assignment {id} retrieved successfully");
    ...
    {
        _logger.LogDebug($"Fetching assignment {id} for batch {batchId}");
        ...
        var dto = _mapper.Map<AssignmentDto>(assignmentEntity);
        _logger.LogInfo($"Assignment {id} retrieved for batch {batchId}");
    ...
    private async Task<Assignment> GetAssignmentForBatch(Guid batchId, Guid id, bool trackChanges)
    {
        ...
        _logger.LogDebug($"Assignment {id} retrieved for batch {batchId}");
    ...
    private async Task EnsureTeacherExistsWithRole(Guid teacherId)
    {
        ...
        if (teacher is null)
        {
            _logger.LogWarn($"Teacher with id {teacherId} not found.");
